...
from draw_line import process_first_frame
    ...
    def vdo_cap(self):
        ...
        try:
            ...
            while True:
                ...
                if not self.line:
                    self.line_coord, self.image = process_first_frame(self.image, self.window_name)
                    self.line = True
                ...
                obj = pool.apply_async(YoloObjectDetection(self.q_img, self.model, self.line_coord, self.line_coord).predict)
                frame = obj.get()
...
if __name__ == "__main__":
    urls = [
        ...
        {"name": "rtsp4", "url": r"rtsp://admin:Admin123$@10.11.25.64:554/stream1"},
        ...
        # {"name": "rtsp12", "url": r"rtsp://admin:Admin123$@10.11.25.65:554/stream1"}
    ]
    queue_list = []
    threads = []
    for i in urls:
